...
import type { BgmCharacterSearchResultItem, BgmSubjectSearchResultItem, BgmPersonSearchResultItem } from '~/types'
...
const keyword = ref('')
const searchResult = shallowRef<(BgmCharacterSearchResultItem | BgmSubjectSearchResultItem | BgmPersonSearchResultItem)[]>([])
...
const searchType = ref<'character' | 'person' | 'game'>('game')
...
function handleAdd(item: BgmCharacterSearchResultItem | BgmSubjectSearchResultItem | BgmPersonSearchResultItem) {
  emit('add', {
    id: item.id,
    name: item.name,
...
<template>
  <div class="flex flex-col gap-4 p-6 h-full">
    ...
    <p class="text-xs text-black px-1 ml-11 font-medium">
      提示：如果搜不到，请尝试输入<b>完整全名</b>或<b>日文</b> 。例如：`圣奴隶学园3`搜不到，就输入`聖奴隷学園3`。<br/>
      或者使用<b>自定义上传</b>功能添加你的图片！
    </p>
    ...
    <div class="flex-1 overflow-y-auto min-h-0">
      ...
      <div v-if="activeTab === 'search'">
        ...
        <div class="flex flex-col gap-2 mb-4 px-1">
          <div class="flex flex-wrap items-center justify-center gap-2">
             <button
               v-for="type in ['game', 'character', 'person']"
               :key="type"
             ...
             >
               {{
                 type === 'game' ? 'Galgame' :
                 type === 'character' ? '角色' :
                 type === 'person' ? '声优/人物' : type
               }}
             </button>
          </div>

          <!-- Secondary Filters (Year) -->
          <div v-if="searchType === 'game'" class="flex items-center justify-center gap-2">
        ...
        <div v-if="searchResult.length" class="columns-2 md:columns-3 lg:columns-4 gap-4 pb-4 space-y-4">
          ...
          >
            ...
            <p v-else-if="'relation' in item && item.relation" class="w-full text-center text-xs text-gray-500 font-medium -mt-1">
               {{ item.relation }}
            </p>
            <p v-else-if="'career' in item && item.career" class="w-full text-center text-xs text-gray-500 font-medium -mt-1">
               {{ item.career?.join(' / ') || '未知' }}
            </p>
          </div>
        </div>
      ...
      <div v-else class="p-4 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-2">
            <label class="text-sm font-bold text-black">角色/作品名字 (可选)</label>
            <input
                v-model="customName"
        ...
        <div class="flex flex-col gap-2">
          <label class="text-sm font-bold text-black">上传Galgame封面或角色图片</label>
          <div
        ...
        >
            ...
            <p class="text-sm text-black font-medium">{{ customImagePreview ? '点击更换图片' : '点击或拖拽上传Galgame图片' }}</p>
          </div>
        </div>
