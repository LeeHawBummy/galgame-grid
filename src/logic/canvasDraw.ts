...
export class CanvasGenerator {
    ...
    private async drawWatermark(width: number, height: number, padding: number) {
        ...
        try {
            // Load and draw title image as watermark
            const titleImage = await this.loadImage('/title_top.png')
            //const logo = await this.loadImage('/logo.png')
            
            const logoSize = 40
            const titleHeight = 40
            const titleWidth = (titleImage.width / titleImage.height) * titleHeight
            
            // Draw logo
            //this.ctx.drawImage(logo, x - logoSize, y - logoSize + 5, logoSize, logoSize)
            
            // Draw title image next to logo
            this.ctx.drawImage(titleImage, x - logoSize - titleWidth - 10, y - titleHeight, titleWidth, titleHeight)
            ...
            this.ctx.fillText('Gal选拔总选举', x, y)
        }
